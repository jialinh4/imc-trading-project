...
def _build_trade_history() -> pd.DataFrame:
    rows = [
        {
            "timestamp": 0,
...
def test_legacy_backtester_smoke_run(tmp_path) -> None:
    ...
    log_file = tmp_path / "smoke_backtest.log"
    ...
    assert len(backtester.pnl_history) == 4
    assert set(backtester.pnl.keys()) == {Product.AMETHYSTS, Product.STARFRUIT}
